...
function runEslint(files) {
  gulp
    .src(files || "src/**/*.{js,html}", { cwd: projectRoot })
    .pipe(eslint())
    .pipe(eslint.format())
    .pipe(
...
function build(dest) {
  dest = dest || "build";
  return gulp
    .src("index.html", { cwd: projectRoot })
    .pipe(
      vulcanize({
        inlineScripts: true,
        inlineCss: true,
  ...
  Q.nfcall(rmdir, dest)
    ...
    .then(function () {
      ...
      return Q.all([
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "background.js"),
          path.resolve(dest, "background.js")
        ),
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "cordova.js"),
          path.resolve(dest, "cordova.js")
        ),
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "overrides.css"),
          path.resolve(dest, "overrides.css")
        ),
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "manifest.json"),
          path.resolve(dest, "manifest.json")
        ),
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "src/crypto.js"),
          path.resolve(dest, "src/crypto.js")
        ),
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "lib/sjcl.js"),
          path.resolve(dest, "lib/sjcl.js")
        ),
        Q.nfcall(
          ncp,
          path.resolve(projectRoot, "assets"),
          path.resolve(dest, "assets")
        )
      ]);
    })
    .then(function () {
