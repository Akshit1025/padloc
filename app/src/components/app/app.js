  ...
  return Polymer({
    ...
    init: function (collection, settings, announcements) {
      ...
      this.collection.addEventListener(
        ...
        function (e) {
          ...
          e.detail.slice(2).forEach(
            function (record) {
              // Add category to `_categories` list in case it is not there yet
              if (
                record.category &&
                this._categories.indexOf(record.category) == -1
              ) {
    ...
    _openView: function (view, inOpts, outOpts) {
      ...
      if (this._currentView) {
        ...
        this.async(
          this._currentView.hide.bind(this._currentView, outOpts),
          outOpts.delay || 0
        );
      } else {
        view.show(inOpts);
    ...
    _addRecord: function () {
      this._openForm(
        ...
        "Add Record",
        this._confirmAddRecord.bind(this)
      );
    },
    ...
    _keydown: function (event) {
      ...
      // DOWN -> Mark next
        ...
        shortcut =
          this._currentView.markNext &&
          this._currentView.markNext.bind(this._currentView);
      }
      // UP -> Mark previous
      else if (event.keyCode == 38 && !dialogsOpen) {
        shortcut =
          this._currentView.markPrev &&
          this._currentView.markPrev.bind(this._currentView);
      }
      // ENTER -> Select marked
      ...
      // CTRL/CMD + N -> New Record
        ...
        shortcut =
          this._currentView.add &&
          this._currentView.add.bind(this._currentView);
      }
    ...
    _requireRemotePassword: function () {
      this._openForm(
        ...
        "It seems your local master password does not match the one used to encrypt " +
          "the data on Padlock Cloud. Please enter your remote password!",
        function (vals) {
    ...
    _notifyHeaderIcons: function () {
      this.notifyPath(
        "_currentView.leftHeaderIcon",
        this._currentView && this._currentView.leftHeaderIcon
      );
      this.notifyPath(
        "_currentView.rightHeaderIcon",
        this._currentView && this._currentView.rightHeaderIcon
      );
    },
    ...
    _displayShortcuts: function () {
      this._openForm([
        {
          element: "button",
          label: "[Esc] \u279e Back / Open Menu",
          submit: true
        },
        {
          element: "button",
          label: "[ctrl/cmd] + F \u279e Find Record",
          submit: true
        },
        {
          element: "button",
          label: "[ctrl/cmd] + N \u279e Create New",
          submit: true
        },
        {
          element: "button",
          label: "[ctrl/cmd] + L \u279e Lock App",
          submit: true
        },
        {
          element: "button",
          label: "[ctrl/cmd] + S \u279e Synchronize",
          submit: true
        },
        {
          element: "button",
          label: "[\u2193] \u279e Mark Next Item",
          submit: true
        },
        {
          element: "button",
          label: "[\u2191] \u279e Mark Previous Item",
          submit: true
        },
        {
          element: "button",
          label: "[Enter] \u279e Select Marked Item",
          submit: true
        }
      ]);
    },
