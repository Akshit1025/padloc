...
export function init(
  cl: Client,
  st?: { get(): Promise<any>; set(data: any): Promise<void> }
) {
  client = cl;
  statsApi = st || {
    get() {
...
let ready = initialized
  .then(() => Promise.all([statsApi.get(), getAppVersion()]))
  .then(([stats, version]) => {
    trackingID = stats.trackingID as string;
    const launchCount =
      typeof stats.launchCount === "number"
        ? (stats.launchCount as number) + 1
        : 1;
  ...
  ready = ready
    ...
    .then((stats) => {
      ...
      if (stats.lastAskedFeedback) {
        Object.assign(data.props, {
          "Last Rated": new Date(
            stats.lastAskedFeedback as number
          ).toISOString(),
