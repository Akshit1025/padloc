define(["padlock/Settings"], function (Settings) {
  ...
  test("Set/get settings", function () {
    ...
    equal(
      ...
      "some value",
      "Once a setting has been set, it should be retrieveable via the get method"
    );
    equal(
      settings.get("setting_2"),
      "default",
      "If a setting has not been set, the default value should be return if there is one"
    );
    equal(
      settings.get("setting_3"),
      undefined,
      "Trying to get an unset setting should return undefined"
    );
  });
  ...
  test("Predefined settings", function () {
    ...
    ok(
      settings.hasOwnProperty("setting_1"),
      "A property should have been defined for the predefined property"
    );
    equal(
      settings.setting_1,
      "default",
      "The property should have the default value initially"
    );
        ...
        settings.fetch({
          success: function () {
            equal(
              settings.setting_1,
              "some value",
              "The settings object should contain the saved setting with the correct value"
            );
            start();
          }
        });
