...
require([
  ...
  "padlock/model",
  "padlock/import"
], function (crypto, util, model, imp) {
  ...
  test("key generation", function () {
    ...
    equal(
      p.key,
      p2.key,
      "Two keys created with the same password and salt should be equal."
    );
    equal(
      p.salt,
      p2.salt,
  ...
  test("encrypt/decrypt roundtrip", function () {
    ...
    notEqual(
      newC.ct,
      c.ct,
      "Encrypting the same value twice with the same key should result " +
  ...
  test("mixin", function () {
    ...
    equal(
      d.one,
      "one",
      "If overwrte is true, overwrite existing properties in target object"
    );
  });
  ...
  test("add records", function () {
    ...
    var rec2 = { name: "Orang", value: "Utan" },
      rec3 = { name: "John", value: "Travolta" };
    coll.add([rec2, rec3]);
  ...
  test("save collection", function () {
    ...
    coll.save();
    notEqual(
      localStorage.getItem("coll_" + collName),
      null,
      "There should be something in the localStorage now."
    );
    ...
    deepEqual(
      newColl.records,
      coll.records,
      "After fetching, the collection should be populated with the correct records."
    );
  });
  ...
  test("fetch an existing collection", function () {
    ...
    ok(
      !coll2.fetch("drowssap"),
      "Fetching with a wrong password should return false"
    );
    ok(
      coll2.fetch(password),
      "Fetching with the correct password should return true"
    );
    deepEqual(
      coll.records,
      coll2.records,
      "After fetching, the collection should contain the correct records"
    );
  });
  ...
  test("test if a collection exists", function () {
    ...
    ok(
      !coll.exists(),
      "Collection has not been saved yet, so it shouldn't exist in the store"
    );

    coll.save();
    ok(
      coll.exists(),
      "After the collection has been saved, it should exist ist the store."
    );
  });
  ...
  test("set/get category", function () {
    ...
    equal(
      ...
      color,
      "Categories object should contain added category with correct value"
    );
    equal(
      categories.get("not there"),
      undefined,
      "Looking up a non-existent category should return undefined"
    );
  });
  ...
  test("save/fetch categories", function () {
    ...
    equal(
      categories.get(catName),
      color,
      "The set should contain the saved category with the correct value"
    );
    equal(
      categories.get(catName2),
      color3,
      "If a category name is already present in the set, the old value should be kept instead of overwriting it"
    );
  });
  ...
  test("Get preferable color for a new category", function () {
    ...
    ok(
      color > 0 && color <= categories.numColors,
      "Returned number should be in range of available colors."
    );
  });
      ...
      expected = [
        {
          ...
          fields: [{ name: "My field", value: "some value" }]
        },
        {
          name: "Website",
          category: "Import Test",
