...
async function testSource(source: Source, testClear = true) {
  ...
  assert.equal(
    data,
    "test",
    "After setting data, get should return the same data"
  );
  if (testClear) {
    await source.clear();
    data = await source.get();
    assert.equal(
      data,
      "",
      "After clearning data, get should return empty string"
    );
  }
}
...
suite("source", () => {
  ...
  test("EncryptedSource", async () => {
    ...
    await source.clear();
    assert(
      !(await source.hasData()),
      "hasData for empty source should return with `false`"
    );
    source.password = "secret";
    await source.set("test");
    assert(
      await source.hasData(),
      "hasData for non-empty source should return with `true`"
    );
  });
