...
define(["padlock/crypto", "padlock/util"], function (crypto, util) {
  ...
  var isSecuStoreBackup = function (rawData) {
    ...
    return (
      rawData.indexOf("SecuStore") != -1 &&
      rawData.indexOf("#begin") != -1 &&
      rawData.indexOf("#end") != -1
    );
  };
  ...
  var importSecuStoreBackup = function (rawData, password) {
    ...
    try {
      ...
      console.error(
        "Failed to restore backup! The data is corrupt or password incorrect!",
        e
      );
      return null;
    }
