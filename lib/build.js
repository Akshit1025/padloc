...
function compileCore(watch = false) {
  ...
  function bundle() {
    ...
    return stream.pipe(source("padlock.js")).pipe(gulp.dest(`${appDir}/src/`));
  }
...
function copyFiles(source, dest, files) {
  return Promise.all(
    files.map((f) => copy(path.resolve(source, f), path.resolve(dest, f)))
  );
}
/**
...
function bundle(dest = "build") {
  ...
  return rmdir(dest)
    ...
    .then(() =>
      pexec(`node ${bundler} ${flags} index.html > ${dest}/index.html`)
    );
}

function buildChrome() {
  const { version, description, author } = require(
      ...
      () =>
        new Promise((resolve, reject) => {
          let output = fs.createWriteStream(
            path.join(dest, `padlock-v${version}-chrome.zip`)
          );
