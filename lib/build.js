...
const { name, version, description, author, licence, homepage } = require(
...
function buildChrome() {
  ...
  return build(buildDir)
    .then(() =>
      copyFiles(path.join(projectRoot, "chrome"), buildDir, [
        "manifest.json",
        "background.js",
        "icons"
      ])
...
function buildMac() {
  return build("dist/electron")
    .then(() =>
      copyFiles("electron", "dist/electron", ["main.js", "package.json"])
    )
    .then(() =>
      builder.build({
