      ...
      enter(node) {
        if (
          ...
          try {
            ...
            console.warn(
              `${e.message} (${l.file}, ${l.start.line}:${l.start.column})`
            );
          }
        }
      }
...
function saveLanguageFile(path, dict) {
  const translations = Object.entries(dict).map((e) => {
    return { o: e[0], t: e[1] };
  });
...
function updateLanguageFiles(
  srcRoot = "./index.html",
  targetDir = "resources/translations",
  languages = ["en", "de"],
  funcName = "$l"
) {
...
function buildTranslationsFile(
  srcDir = "resources/translations",
  dest = "translations.js",
  languages = ["en", "de"]
) {
  const translations = {};
